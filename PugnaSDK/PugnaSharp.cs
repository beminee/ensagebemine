namespace PugnaSharpSDK
{
    ...
    public class PugnaSharp : KeyPressOrbwalkingModeAsync
    {
        ...
        public override async Task ExecuteAsync(CancellationToken token)
        {
            ...
            var allTargets = TargetSelector.Active.GetTargets().FirstOrDefault();
            ...
            {
                Log.Debug("Post drain hp is higher than your hp threshold to begin healing, please change this or the script won't work.");
                return;
            }
            ...
            try
            {
                if (!silenced)
                {
                    ...
                    if (HealTarget != null)
                    {
                        ...
                        if (HealTarget != null && IsHealing && (HealTarget.HealthPercent() >= ((float)allyPostDrain / 100)))
                        {
                            Owner.Stop();
                            IsHealing = false;
                        }
            ...
            if (!silenced && target != null)
            {
                ...
                EntityManager<Hero>.Entities.Where(
                     x => x.IsValid && x.Team != this.Owner.Team && !x.IsIllusion && !UnitExtensions.IsMagicImmune(x) && x.Distance2D(this.Owner) <= Ward.GetAbilityData("radius"))
                         .ToList();
                ...
                if (target != null)
                {
                        ...
                        && target.IsAlive && target != null)
                    {
                        ...
                        EntityManager<Hero>.Entities.OrderBy(x => x == allTargets).Where(
                            x => x.IsValid && x.IsVisible && x.Team != Owner.Team && !x.IsIllusion && !UnitExtensions.IsMagicImmune(x)).ToList();
                        var input =
                            new PredictionInput(
                                Owner,
                                target,
                        ...
                        if (output.HitChance >= HitChance.Medium)
                        {
                            ...
                            await Await.Delay(GetAbilityDelay(target, this.Blast), token);
                        }
                    }
        ...
        public virtual async Task KillStealAsync()
        {
            ...
            bool comboMana = Blast.GetManaCost(Blast.Level - 1) + Decrepify.GetManaCost(Decrepify.Level - 1) < Owner.Mana;
